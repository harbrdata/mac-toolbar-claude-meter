...
Displays a color-coded ring gauge in the menu bar showing your current 5-hour usage window, with your 7-day usage as a muted arc behind it. Click it for a full breakdown of all usage windows with progress bars and reset countdowns, plus your remaining usage credits.
...
The installer copies the app to `/Applications`, strips the macOS quarantine flag, and launches it.

To upgrade, repeat the same steps — the installer replaces the existing app.
...
- Optional **Show Credits Dial** mode — an extra gauge for usage-credit spend against your monthly cap
- Dropdown with all usage windows: 5h, 7d, Opus, Sonnet, Cowork, OAuth, plus usage credits (percentage spent and balance remaining) under the 7d window
- Progress bars and reset time countdowns
- Configurable refresh interval (1m / 2m / 5m / 10m)
- Separate usage alert notifications for the 5h window, the 7d window and usage credits, each with its own threshold (75% / 80% / 85% / 90% / 95% / Off; defaults 95%, 80% and 80%)
